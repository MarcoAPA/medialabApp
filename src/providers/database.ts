...
export class Database {
    ...
    public dropE(){
      return new Promise((resolve, reject) => {
        this.storage.executeSql("DROP TABLE events", []).then((data) => {
            ...
            //alert('Unable to drop Events table: '+ JSON.stringify(err));
        }); 
      });
    }
    ...
    public dropAPPI(){
      return new Promise((resolve, reject) => {
        this.storage.executeSql("DROP TABLE appinfo", []).then((data) => {
            ...
            reject(error);
            console.error('Unable to execute sql: ', error);
            //alert('Unable to drop appInfo table: '+ JSON.stringify(err));
        }); 
      });
    }
    ...
    public createEventTable(){
      return new Promise((resolve, reject) => {
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS events (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, description TEXT, place TEXT, pagurl TEXT, etype TEXT, d DATE)", []).then((data) => {
            ...
            reject(error);
            console.error('Unable to execute sql: ', error);
            //alert('Unable to create table events: '+ JSON.stringify(err));
        });  
      });
    }
    ...
    public createAppInfoTable(){
      return new Promise((resolve, reject) => {
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS appinfo (id INTEGER PRIMARY KEY AUTOINCREMENT, year INTEGER, month INTEGER, day INTEGER)", []).then((data) => {
            ...
            reject(error);
            console.error('Unable to execute sql: ', error);
            //alert('Unable to create table appinfo: '+ JSON.stringify(error));
        });
      });
    }
      ...
      return new Promise((resolve, reject) => { 
        ...
        this.getInfo().then((result) => {
            ...
            if( currentDate.year > lastupdate.year ){
              this.getXLSRequest();
            } else if ( currentDate.year == lastupdate.year && currentDate.month > lastupdate.month){
              this.getXLSRequest();
            } else if ( currentDate.year == lastupdate.year && currentDate.month == lastupdate.month && currentDate.day > lastupdate.day ){
              this.getXLSRequest();
            }
    ...
    /*
    Método que devuelve las últimas fechas de acceso al xls
     */
    public getInfo() {
      return new Promise((resolve, reject) => {
